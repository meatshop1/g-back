                    ...
                    sshagent(['aws-dev-deploy']) {
                        sh '''
                            ssh -o StrictHostKeyChecking=no ubuntu@157.175.177.0 "
                                ...
                                sudo docker run -d --network meatshop-net -e DJANGO_SETTINGS_MODULE=meatshop.settings.prod -e LOCAL_DB_NAME=meatshop -e LOCAL_DB_PORT=3306 -e LOCAL_DB_HOST=mymysql -e LOCAL_DB_USER=root -e LOCAL_DB_PASSWORD=mypass -p 80:8000 --name backend eladwy/backend:$GIT_COMMIT
                            "
                        '''
                    }
                }
